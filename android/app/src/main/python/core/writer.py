...
### 防AI味
- 禁用: 太阳穴/胸口/心跳/呼吸/脊背/瞳孔/冷汗/浑身一震/嘴角勾起/微微上扬/眼底闪过/眸光/眸色/眸子
- 禁用: 「不是A而是B」「真正的/本质上/关键在于」「与此同时/值得一提的是」「随着……的发展」
- 句长有变化，别每句都15字上下
- 少用「似乎/仿佛/或许/大概」

### 活人感（v2.3.7 · 最高优先级的防 AI 味规则 — 真人不会面面俱到）
...
### 人物关系铺垫（v2.4.3 · 禁止"自来熟"）
- **初次见面 = 陌生人**：与未铺垫过的角色初遇，必须写陌生感——试探、戒备、信息交换；禁止直接热络
- **信任要用事件换**：共患难/救命/利益绑定之后才可亲近；禁止"一见如故"（除非设定明确是旧识/系统强制）
- **称呼与信任同步**：全名/敬称 → 外号 → 直呼其名，随着关系进展渐变；禁止第一次见面就叫昵称
- **反派的"知道"要有来源**：主角的身份/秘密被识破时，必须交代来源（被出卖/被调查/特殊能力）
- **关系梯度**：每章只推进一级关系（陌生→认识→合作→信任），禁止一章内从陌生跳到生死之交
