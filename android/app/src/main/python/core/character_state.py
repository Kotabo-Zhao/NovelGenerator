...
5. **跨章状态追踪（P0 级，必须提取）**：本角色在本章新获得的「跨章必须记住的状态」写入 persistent_changes——
   包括但不限于：
   - **永久/长期状态**：失明/断臂/毁容/中毒/诅咒/血脉觉醒/境界跌落的后遗症/被夺走的物品/不可逆的伤势
   - **未愈伤势（v2.4.5 新增）**：摔伤、磕破、划伤、擦伤、扭伤、烫伤、流血——只要本章结束时**尚未痊愈、仍影响角色行动或形象**（走路疼、行动不便、身上带伤疤/血迹/绷带），就必须提取。典型例子：'膝盖摔伤破皮渗血，走路跛行'、'嘴唇磕破结痂'。
   若本章明确解除了某个已有跨章状态（如伤势痊愈、诅咒解除），写入 persistent_resolved。
   **不要提取**：瞬间即逝的短暂状态（如本章内已处理完的擦伤、短暂疲惫、一次性惊吓）——它们在本章结束前已无影响，下一章不需要记住。
...
{{
  "protagonist": {{
    "persistent_changes": ["新获得的跨章状态，如：左眼失明（毒针所致，无解）、膝盖摔伤破皮未愈（影响行动）"],
    "persistent_resolved": ["本章解除的跨章状态，如：右臂旧伤已痊愈"],
  ...
  "other_characters": {{
    "角色名": {{
      "persistent_changes": ["新获得的跨章状态（含未愈伤势）"],
      "persistent_resolved": ["解除的跨章状态"],
      "appeared": true,
      "identity": "身份",
      "location": "出现位置",
      "health_change": "状态变化",
