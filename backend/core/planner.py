...
## 创意与差异化（v3.5.26: 可选增强，默认不启用——忠实执行用户需求优先）

**第一原则：用户明确描述的设定必须原样保留，优先级最高。不得用任何创意策略覆盖、替换或稀释用户需求。**
以下策略**默认不执行**。仅当用户灵感中明确表达了"创意/脑洞/反套路/与众不同/有新意/再大胆一点"等诉求时，才参考其中的策略。

### 1. 反套路金手指
- 拒绝「废材→捡宝物→变强→换地图」的无限循环
- 金手指必须有代价：力量越大代价越致命（而非「无敌流」零风险）
- 跨界知识降维：用统计学/物理学/工程学/法医学等真实学科改造传统修仙/奇幻设定
- 例子：用热力学分析灵气、用博弈论破修真界政治、用法医解剖修士尸体

### 2. 高概念设定（一句话能火的那种）
- 概念必须用一句话说清楚且让人立刻觉得有趣
- 检验标准：说出来朋友会问「然后呢？」
- 拒绝同质化：不是「又一个修仙的」而是「修仙的，但……」

### 3. 情绪锚点
- 找到当代读者最焦虑/最渴望的东西：内卷疲惫→发疯反抗、意义缺失→家族传承、不确定性→信息差优势
- 开局必须有强烈的情绪驱动：不是「我要变强」而是「我不做这件事我家人今晚就会死」
- 拒绝「主角想变强」这种弱动机，代之以「被迫卷入绝境」「复仇」「守护某人」「活下去」

### 4. 三秒钩（开局即冲突）
- 第一章/前三章不要铺设定，直接从冲突中段开始
- 第一段必须让读者产生「怎么会这样」「接下来怎么办」的疑问
- 检验：删掉前三章的全部设定性描述，故事还能不能读？如果不能，重写。

### 5. 差异化题材选择
- 玄幻修仙可以写，但必须是「修仙+X」：+民俗悬疑（捞尸人）、+统计分析（谁让他修仙的）、+法医（验尸修仙）
- 历史可以写，但必须是「历史+反常识视角」：非帝王将相视角（小兵/匠人/商人/翻译官）
- 都市可以写，但必须有「日常中的异常」：普通人突然获得一个无法控制的诡异能力
- 拒绝「穿越者靠现代知识碾压古人」这种已经烂掉的设定
...
只输出JSON。"""
    ...
    async def _call_llm(self, prompt: str, phase: str, max_tokens: int = 4096, retry: bool = True) -> dict:
        ...
        retry_plan = [
            ...
            (0.4, 5, False),   # 第二次：低温度，等5s（覆盖短时网络波动）
            (0.2, 10, True),   # 第三次：最低温 + 简化prompt兜底，等10s
        ]
