...
### 3. 段落格式
- 一段 = 3-5句组成的语义单元。绝对不要一句话一段！
- 禁止把每句都用空行隔开。这是AI最明显的特征。
- 对话场景的连续对话在同一段内，不要每句话一段。
...
### 8. 尺度保真（v2.6）
- 原文中的亲密场景、身体接触、欲望张力描写，改写时**保留原尺度**，不得弱化、删减或"文学化"成含蓄表达
- 只改表达方式的"人味儿"，不改内容的露骨程度

## 输出
直接输出改写后的文字。使用正常段落格式（3-5句一段，段间空一行）。保持原意，但用"人"的方式写。只输出改写内容，不要说明。"""
