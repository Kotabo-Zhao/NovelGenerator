...
### 角色一致性（P0级 — 致命错误）
- 已死亡的角色是否突然出现？
- 角色的性格、能力、知识是否与设定一致？（**例外**：符合角色成长弧线且本章有事件触发的性格演变不算矛盾，如受重大打击后从天真变冷硬）
- 角色之间的关系是否前后矛盾？（**例外**：关系升级/恶化若有事件铺垫不算矛盾）
- **亲属关系常识（P0级）：父子/母子/兄妹/夫妻/师徒等亲属的姓氏、称呼、辈分是否一致
  （如皇帝姓赵则太子/皇子必须姓赵，除非文中明确说明改姓/收养/赐姓等特殊设定）；
  兄弟姐妹长幼顺序、夫妻称谓、长辈晚辈用词是否自洽**

### 情节逻辑（P0/P1级）
- 事件因果关系是否合理？
- 角色的决策是否符合其性格和处境？
- 能力/道具的使用是否有被遗忘或滥用？
- **人物关系铺垫（P1级）：主角与角色互动时，若双方此前无相识过程铺垫
  （前文未出现、初次见面即热络/托付生死/互称昵称/反派凭空识破主角身份），
  应标注"关系突兀"并指出缺失的铺垫环节。
  **例外（不算违规）**：重生/穿越继承记忆、魂穿继承原主关系网、
  系统/规则强制绑定关系、前文明确铺垫的旧识、文中刚发生共患难事件作为速建信任依据
